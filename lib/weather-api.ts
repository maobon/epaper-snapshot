...
const weatherCacheGlobal = globalThis as typeof globalThis & {
  __weatherEpaperResponseCache?: Map<string, WeatherCacheEntry>;
};
...
async function fetchWeatherData<T>(endpointName: WeatherEndpoint, city: WeatherCity): Promise<T> {
  const cacheKey = `${city.cityId}:${endpointName}`;
  const cached = responseCache.get(cacheKey);
  if (cached && cached.expiresAt > Date.now()) return cached.promise as Promise<T>;

  const promise = fetchWeatherDataUncached<T>(endpointName, city);
  const expiresAt = (Math.floor(Date.now() / cacheTtl) + 1) * cacheTtl;
  responseCache.set(cacheKey, { expiresAt, promise });
  promise.catch(() => {
...
async function fetchWeatherDataUncached<T>(endpointName: WeatherEndpoint, city: WeatherCity): Promise<T> {
...
export function fetchHourlyWeather<T>(city: WeatherCity) {
  return fetchWeatherData<T>('hw-hourly', city);
}

export function fetchDailyWeather<T>(city: WeatherCity) {
  return fetchWeatherData<T>('hw-daily', city);
}

export function fetchDaysWeather<T>(city: WeatherCity) {
  return fetchWeatherData<T>('hw-days', city);
}
