...
function seriesPoints(values: number[], top: number, bottom: number) {
  ...
  return values.map((value, index) => ({ x: 30 + index * 66, y: bottom - ((value - min) / span) * (bottom - top) }));
}
      ...
      <section className="epaper-portrait flex h-[800px] w-[480px] flex-col gap-1.5 overflow-hidden rounded-none border-2 border-black bg-white p-2" aria-label={`Portrait ${dashboard.city} weather dashboard`}>
        <section className="h-[184px] shrink-0 overflow-hidden rounded-2xl border border-[#555] bg-white p-2" aria-label="Day weather">
          ...
          <div className="flex h-[82px] items-center">
            ...
            <div className="ml-2 flex w-[116px] flex-col items-center"><DayIcon color="#555" size={43} weight="light" /><span className="mt-0.5 text-center text-[11px] leading-none font-semibold uppercase">{dashboard.day.condition}</span></div>
            <dl className="ml-auto w-[142px] space-y-2 text-xs"><div className="flex items-center gap-1.5"><WindIcon size={16} weight="light" /><dt className="sr-only">Wind</dt><dd><b>Wind</b> {dashboard.day.wind} {dashboard.windUnit}</dd></div><div className="flex items-center gap-1.5"><WindIcon className="rotate-180" size={16} weight="light" /><dt className="sr-only">Wind gust level</dt><dd><b>Gust</b> · Level {dashboard.day.gustLevel}</dd></div></dl>
          </div>
        ...
        <section className="h-[96px] shrink-0 rounded-2xl border border-[#555] bg-white p-2" aria-label="Night weather">
          ...
          <div className="flex h-[58px] items-center"><div className="flex w-[112px] items-start"><strong className="text-[56px] leading-none tracking-[-3px]">{dashboard.night.temp}</strong><span className="text-2xl leading-none">°</span></div><div className="flex w-[126px] items-center gap-2"><NightIcon color="#555" size={35} weight="light" /><span className="max-w-[80px] text-[10px] leading-tight font-semibold uppercase">{dashboard.night.condition}</span></div><dl className="ml-auto w-[142px] space-y-1.5 text-xs"><div className="flex items-center gap-1.5"><WindIcon size={16} weight="light" /><dt className="sr-only">Wind</dt><dd><b>Wind</b> {dashboard.night.wind} {dashboard.windUnit}</dd></div><div className="flex items-center gap-1.5"><WindIcon className="rotate-180" size={16} weight="light" /><dt className="sr-only">Wind gust level</dt><dd><b>Gust</b> · Level {dashboard.night.gustLevel}</dd></div></dl></div>
        </section>
        ...
        <section className="h-[234px] shrink-0 overflow-hidden rounded-2xl border border-[#555] bg-white p-2" aria-label="Seven day weather forecast">
          ...
          <div className="relative h-[72px]"><TemperatureLine color="#000" points={highPoints} /><TemperatureLine color="#aaa" points={lowPoints} />{dashboard.forecast.map((item, index) => <div className="absolute top-0 flex h-[72px] w-[52px] flex-col justify-between text-center text-[10px] font-semibold" key={`temperature-${item.date}`} style={{ left: index * 66 + 4 }}><span>{item.high}°</span><span className="text-[#555]">{item.low}°</span></div>)}</div>
